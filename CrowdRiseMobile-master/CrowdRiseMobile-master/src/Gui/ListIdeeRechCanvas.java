 ...
 * @author userpc
 */
public class ListIdeeRechCanvas extends Canvas implements CommandListener,Runnable{
    ...
    String mot = RechercheIdeeForm.str;
    
    public ListIdeeRechCanvas(Display d) {
       this.disp=d;
        ...
        switch (getGameAction(keyCode)) {
            ...
            case (DOWN):
               if (pos==1 && pos<y) {
                ...
                break;
            case (FIRE):
                System.out.println("HERE idée = "+idee[indice]);
    ...
    public void commandAction(Command c, Displayable d) {
        if (c == cmdBack) {
            disp.setCurrent(new RechercheIdeeForm(disp));
        }
    ...
    public void run() {
        try {
            ...
            IdeesHandler ideesHandler = new IdeesHandler();
                ...
                HttpConnection hc;
                if ((Midlet.currentPosition.equals("listIdées"))||(Midlet.currentPosition.equals("listIdéesAdmin"))) {
                  hc = (HttpConnection) Connector.open("http://localhost/crowdrise/idees/rech_idee.php?mot=" + mot);  
                }else{
                  hc = (HttpConnection) Connector.open("http://localhost/crowdrise/idees/rech_mes_idee.php?mot=" + mot+"&membre="+Midlet.membre.getId());
                }
